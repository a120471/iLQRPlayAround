...
def plot_result(xs_in, us_in, xs_out, us_out):
  # plt.figure()
  # plt.title('origin pos, heading and speed')
  # plt.plot(xs_in[:,0], xs_in[:,1], 'r')
  # for i in range(len(us_in)):
  #   scale_h = 0.1
  #   scale_v = 0.1
  #   plt.plot(np.array([xs_in[i,0], xs_in[i,0] + np.cos(xs_in[i,2]) * scale_h]),
  #            np.array([xs_in[i,1], xs_in[i,1] + np.sin(xs_in[i,2]) * scale_h]), 'g')
  #   plt.plot(np.array([xs_in[i,0], xs_in[i,0] + np.cos(xs_in[i,2]) * us_in[i,0] * scale_v]),
  #            np.array([xs_in[i,1], xs_in[i,1] + np.sin(xs_in[i,2]) * us_in[i,0] * scale_v]), 'b')
